...
class Space
  ...
  def self.create_space(name:, description:, price:, host:)
    result = DatabaseConnection.query("INSERT INTO spaces (name, description, price, host) VALUES('#{name}', '#{description}', '#{price}') RETURNING id, name, description, price ")
  ...
  def self.view_my_spaces(host)
    result = DatabaseConnection.query("SELECT * FROM spaces WHERE HOST = '#{host}';")
